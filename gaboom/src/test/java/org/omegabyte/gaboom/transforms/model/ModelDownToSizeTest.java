...
public class ModelDownToSizeTest {
    ...
    public void testModelDownToSize() {
        ...
        PAssert.that(output).satisfies((SerializableFunction<Iterable<KV<String, Individuals<String>>>, Void>) kvs -> {
            ...
            assertEquals(10, result.getValue().getIndividuals().size());
            ...
            assertEquals("l7V350", ind.getId());
            assertEquals("poqflsoly", ind.getGenome());
            assertEquals((Double) 48.0, ind.getFitness());
            ...
            assertEquals("RJF1Sj", ind.getId());
            assertEquals("cbosyqqkc", ind.getGenome());
            assertEquals((Double) 77.0, ind.getFitness());

            ind = result.getValue().getIndividuals().get(4);
            assertEquals("n6nDPq", ind.getId());
            assertEquals("powflsomo", ind.getGenome());
            assertEquals((Double) 63.0, ind.getFitness());

            ind = result.getValue().getIndividuals().get(5);
            assertEquals("7vOGJI", ind.getId());
            assertEquals("zxkgxtsps", ind.getGenome());
            assertEquals((Double) 73.0, ind.getFitness());

            ind = result.getValue().getIndividuals().get(6);
            assertEquals("cPNtYF", ind.getId());
            assertEquals("wmrlfxblg", ind.getGenome());
            assertEquals((Double) 80.0, ind.getFitness());

            ind = result.getValue().getIndividuals().get(7);
            assertEquals("l7V350", ind.getId());
            assertEquals("poqflsoly", ind.getGenome());
            assertEquals((Double) 48.0, ind.getFitness());

            ind = result.getValue().getIndividuals().get(8);
            assertEquals("KNrHVY", ind.getId());
            assertEquals("egqflaqlo", ind.getGenome());
            assertEquals((Double) 73.0, ind.getFitness());

            ind = result.getValue().getIndividuals().get(9);
            assertEquals("n6nDPq", ind.getId());
            assertEquals("powflsomo", ind.getGenome());
            assertEquals((Double) 63.0, ind.getFitness());
            return null;
